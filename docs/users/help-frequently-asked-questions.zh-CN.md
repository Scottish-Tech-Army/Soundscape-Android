...
Soundscape 的功能和优势涵盖多种场景和时间跨度。Soundscape 对您的价值可能会随时间变化，因此您今天的使用方式可能与三个月后有所不同。人们常常以*这个应用能解决什么问题？*的角度来看待应用。Soundscape 当然可以用于特定需求——例如在前往目的地途中追踪目的地位置、从地铁站出来时帮助您辨别方向、下车后确定方位，或者查找下一个路口的街道名称和距离。然而，Soundscape 背后的理念是*用声音照亮您的世界*——它的设计初衷是在您外出时随时使用，提供对周围环境的感知：您所在街道的名称、您的行进方向，以及您经过的商家名称。在这种使用模式下，我们的用户将 Soundscape 称为*不错的伴侣应用*，它支持*偶然发现*，能*填补心理地图中的空白*，并在*步行时带来更多信心*。以下是我们的用户将 Soundscape 融入生活的一些其他例子：
...
### 如何充分利用信标？
您可以把音频信标看作*"耳朵的灯塔"*：它会告诉您目的地相对于当前位置在哪个方向（按直线计算）。像灯塔一样，它并不告诉您如何到达——途中您可能需要做出导航选择以绕开障碍。音频信标持续的节奏声从目的地方向传来，帮助您在行走时了解目的地相对于您的方位。当您正朝目的地走去，或将手机指向目的地时，会听到更高音调的*"铃"*声。由于在嘈杂环境中有时难以分辨节奏声的方向，这可以让您准确定位目的地的方向。寻找高音调的*"铃"*声时，请将手机平放并缓慢转身；同时将头部转向与手机相同的方向，可以获得最佳的空间音频体验。

这样的设计有几个自然的结果：

1. 使用信标时并没有*"正确"*的行进方向——使用 Soundscape 时，由您自己选择如何前往；

2. 更高音调的*"铃"*声仅帮助您准确定位目的地的方向——它并不指示您应当怎样前往；
...
为了适应各种路口配置，Soundscape将路口描述为从一个公共点出发的道路段。Soundscape使用空间音频依次指示左侧道路的名称、正前方继续的道路名称以及右侧道路的名称。如果路口描述以您所在的道路开头而不是左侧的道路，那么这是一个T形路口，您所在的道路继续向前延伸，另一条道路从右侧交汇。同样，如果描述只包含左侧和右侧的道路，您就知道您所在的道路在前方的T形路口处终止。即使道路在路口处更名，这种描述路口的方法也能正确工作。

### 为什么Soundscape不会播报我经过的每家商铺？
Soundscape 的设计避免向您播报过多内容。它还使用 OpenStreetMap 作为数据来源。OpenStreetMap（OSM，https://www.openstreetmap.org/）是一个由众多个人贡献者组成的社区共同开发和编辑的世界地图。如果某个商铺或兴趣点没有被 Soundscape 播报，最可能的原因是 OSM 社区成员尚未将该商铺添加到数据中，或在某些情况下尚未更新。

### 为什么某些播报在我乘坐交通工具时停止了？
为了避免播报数量过多，某些类别（如路口）在您乘坐交通工具行驶时不会自动播报。

### 如果我没听清播报或因环境噪音错过了怎么办？
Soundscape有一个最近提示列表，方便您查看可能错过的提示。要找到此列表，请点击Soundscape主屏幕上的搜索栏。在页面底部，有一个*最近提示*部分，您错过的提示会列在其中。

## Soundscape如何运作？

### Soundscape可以在什么手机上运行？
Soundscape目前适用于运行iOS 16或更高版本的iPhone，以及运行Android 11或更高版本的安卓手机。
...
与其他地图应用常提供的逐向导航不同，Soundscape 会在目的地方向播放音频信标，帮助您在增强的环境感知和对目的地位置的了解基础上，以最适合您的方式前往目的地。Soundscape 被设计为在后台运行，这样即便您同时使用逐向导航应用，它也能在您前往目的地的过程中持续提供环境感知信息。
...
2. 停止自动播报：当您不在行进中或已到达目的地时，通常不需要 Soundscape 继续播报周围信息。您无需退出应用，可将 Soundscape 设为小睡模式，它会在您离开时再次唤醒；或者将 Soundscape 设为休眠模式，它会保持关闭状态，直到您选择再次打开。或者，您也可以从菜单中选择*\"设置\"*，在*\"管理提示\"*部分把*\"提示详细程度\"*设为*\"静音\"*。

3. 停止信标：有些情况下尽管已设定目的地，您并不需要开启可听信标。例如您可能已经非常清楚如何到达目的地，但仍希望自动收到关于与目的地距离的更新；或者您可能只在接近目的地时才需要音频信标。无论哪种情况，您都可以通过主屏幕上的*\"静音信标\"*/*\"取消静音信标\"*切换按钮来选择何时听到信标。

如果您仍想与 Soundscape 交互但不想听到自动播报，可在菜单中进入*\"设置\"*屏幕的*\"管理提示\"*部分，把*\"提示详细程度\"*设为*\"静音\"*。或者，如果您暂时不打算使用 Soundscape，可在主屏幕使用*\"休眠\"*按钮将其设为休眠或小睡模式。
...
### 什么是 OpenStreetMap，我们为什么在 Soundscape 中使用它？
Soundscape 使用地图数据来生成语音播报。我们的主要地图数据源是 OpenStreetMap（www.openstreetmap.org），这是一个由贡献者社区共同建设的地图平台。组织和个人都可以使用 OpenStreetMap 的工具改进数据，让自己的场所更易于到达。
