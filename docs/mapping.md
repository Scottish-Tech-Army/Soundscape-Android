...
2. A single server to maintain. `soundscape-backend` is relatively expensive to run and by using the protomaps server instead we no longer require it for Android Soundscape.
